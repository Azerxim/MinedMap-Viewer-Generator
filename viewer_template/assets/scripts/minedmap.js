...
function signIcon(material, kind) {
	function createSignIcon(material, kind) {
		...
		return L.icon({
			iconUrl: `assets/images/icon/${material}_${kind}.png`,
			iconSize,
			popupAnchor,
			shadowUrl: `assets/images/icon/shadow_${kind}.png`,
...
const MinedMapLayer = L.TileLayer.extend({
	...
	createTile: function (coords, done) {
		...
		if (coords.z - this.options.zoomOffset >= 0)
			...
			L.DomUtil.addClass(tile, 'bg-base-100');
		return tile;
	},
	...
	getTileUrl: function (coords) {
		...
		return 'assets/data/'+pathname.data+'/'+this.layer+'/'+z+'/r.'+coords.x+'.'+coords.y+'.png';
	},
});
...
const CoordControl = L.Control.extend({
	...
	onAdd: function (map) {
		this._container = L.DomUtil.create('div', 'leaflet-control-attribution bg-base-100 dark:text-white');
...
function createSign(sign, back) {
	...
	container.style.backgroundImage = `url(assets/images/bg/${sign.material}_${sign.kind}.png)`;
...
async function loadSigns(signLayer) {
	const pathname = parsePathName();
	const response = await fetch('assets/data/'+pathname.data+'/entities.json', {cache: 'no-store'});
...
window.createMap = function () {
	(async function () {
		const pathname = parsePathName();
		const response = await fetch('assets/data/'+pathname.data+'/info.json', {cache: 'no-store'});
		const res = await response.json();
		...
		if (json) {
			for (let one in json.polygons) {
				...
				switch (subdata.type) {
					case 'Circle':
						L.circle(JSON.parse(subdata.coords)[0], {color: subdata.color, fillColor: subdata.color, fillOpacity: 0.2, radius: JSON.parse(subdata.coords)[1]}).addTo(map)
					...
					case 'Polygon':
					case 'Line':
						L.polygon(JSON.parse(subdata.coords), {color: subdata.color}).addTo(map)
							.bindPopup(subdata.popup, {'className': 'customPopup'});
						break;
	
					case 'Markers':
						L.marker(JSON.parse(subdata.coords), {icon: subdata.icon}).addTo(map) // [-z, x]
							.bindTooltip('')
							.bindPopup(subdata.popup, {'className': 'customPopup'});
						break;
	
					case 'Text':
						L.marker(JSON.parse(subdata.coords), {textMarker: true, text: subdata.text}).addTo(map) // [-z, x];";
						break;
				}
			}
