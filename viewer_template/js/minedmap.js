...
const MinedMapLayer = L.TileLayer.extend({
	...
	getTileUrl: function (coords) {
		...
		return 'data/'+name+'/'+this.layer+'/'+z+'/r.'+coords.x+'.'+coords.y+'.png';
	},
});
...
async function loadSigns(signLayer) {
	const name = parsePathName();
	const response = await fetch('data/'+name+'/entities.json', {cache: 'no-store'});
...
window.createMap = function () {
	(async function () {
		const name = parsePathName();
		const response = await fetch('data/'+name+'/info.json', {cache: 'no-store'});
		const res = await response.json();
